...
int tool_gpio_write(const char *args_json, char *result, size_t res_len)
{
	...
	if (rc < 0) {
		snprintf(result, res_len, "{\"error\":\"failed to write '%s': %d\"}", pin_name, rc);
		return rc;
	}

	snprintf(result, res_len, "{\"pin\":\"%s\",\"value\":%d,\"status\":\"ok\"}", pin_name,
		 value);
	return 0;
}
...
int tools_build_json(char *buf, size_t buf_len)
{
	...
	for (size_t i = 0; i < TOOLS_COUNT; i++) {
		...
		n = snprintf(buf + pos, buf_len - pos,
			     ...
			     "}"
			     "}",
			     i > 0 ? "," : "", t->name, t->description, t->parameters_json_schema);
		if (n < 0 || (size_t)n >= buf_len - pos) {
			return -ENOMEM;
		}
		pos += n;
